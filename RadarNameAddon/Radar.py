...
class Radar():
	...
	def DrawArrow(self,x,y,color,name,angle=0):
		...
		text_surface = my_font.render(name, True, (0, 0, 0), Color.WHITE)
		self.screen.blit(text_surface, (p0,p1))
	...
	def UpdateSoldiers(self,data):
		...
		for Soldier in data.soldiers:
			...
			if Soldier.alive:
				...
				self.DrawArrow(Position.x,Position.y,(Color.GREEN,Color.RED)[Enemy],Soldier.name, Yaw)
			else:
				if (Enemy):
